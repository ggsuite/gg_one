...
- **`check/`** — static verification: `analyze`, `format`, `pana`, `package_json_scripts` (TypeScript npm scripts), `npm_logged_in` (npm auth before publish), `no_pubspec_overrides` (a `pubspec_overrides.yaml` must not be in effect when publishing — part of `can publish`; `do publish` deletes the file itself up front). Its static `NoPubspecOverrides.hasLocalizedRefs(dir)` answers the weaker question »does this package still redirect a dependency to a working copy?« — a missing, empty or overrides-free file counts as *not* localized, an unparsable one as localized. `--merge-only` (below) and gg_multi's `do merge` use it as their precondition
...
- **`do/`** — actions that execute with validation: `do_commit`, `do_push`, `do_merge`, `do_publish`, `do_configure_publish`, `do_upgrade`, `do_maintain`, `do_checkout`, `create/`
  - `do_commit` refuses to run inside the workspace's `.master` folder (`tools/master_folder_guard.dart`: any path segment named `.master`, so `<root>/.master/<org>/<repo>` counts) and unless HEAD is on a feature branch (gg_git's `IsFeatureBranch`: neither `main` nor `master`, and not a detached HEAD). Both guards run before every other step. `--force` **bypasses the folder guard** — a master repo can still be fixed in place, and the error names that escape hatch — but it does **not** bypass the branch guard, because the branch is not something the checks can fix. The master workspace only mirrors the repositories; work happens in a ticket workspace (`<root>/tickets/<ticket>/<org>/<repo>`), which is why gg_multi's own commit flows (`do commit`, `do review`, `do cancel-review`) always operate on ticket repos and never hit the guard. Note that gg_multi's internal flows (`do review`, `do cancel-review`, `do add`, `do publish`) pass `force: true`, so they are exempt from the folder guard by construction.
