...
- **`do/`** — actions that execute with validation: `do_commit`, `do_push`, `do_merge`, `do_publish`, `do_configure_publish`, `do_upgrade`, `do_maintain`, `do_checkout`, `create/`
  - `do_commit` refuses to run unless HEAD is on a feature branch (gg_git's `IsFeatureBranch`: neither `main` nor `master`, and not a detached HEAD). The guard runs before every other step and `--force` does **not** bypass it — the branch is not something the checks can fix.
