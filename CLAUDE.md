...
- **`do/`** — actions that execute with validation: `do_commit`, `do_push`, `do_publish`, `do_configure_publish`, `do_upgrade`, `do_maintain`, `do_checkout`, `create/`
  ...
  - `do_merge` is **no longer a CLI command** — `gg do merge` was folded into `gg do publish --merge-only` (below), so there is exactly one flow. The `DoMerge` class (`do/do_merge.dart`, still exported because it is the merge implementation `do_publish` drives) has no `argParser` and no CLI surface left; every parameter arrives from `DoPublish`. It drops the `.gg/ticket.json` ticket marker (and its `.gitignore` whitelist) before merging, so it never lands on the main branch. `do_publish` calls the same removal (`DoMerge.removeTicketJson`) before the version bump and registry upload — the upload happens before the merge, so the merge-time removal alone would ship the marker to pub.dev/npm inside the published package.
